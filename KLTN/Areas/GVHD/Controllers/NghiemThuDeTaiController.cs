...
namespace KLTN.Areas.GVHD.Controllers
{
    ...
    public class NghiemThuDeTaiController : Controller
    {
        ...
        {
            List<DeTaiNghienCuu> listDetaiNghiemThu = (from t0 in _context.DeTaiNghienCuu
            ...
            for (int i = 0; i < listDetaiNghiemThu.Count(); i++)
            {
                ...
                obj.IdDeTai = listDetaiNghiemThu[i].Id;
                obj.TenDeTai = listDetaiNghiemThu[i].TenDeTai;
                obj.TinhTrangDeTai = listDetaiNghiemThu[i].TinhTrangDeTai.Value;
                obj.TinhTrang = "Chưa đánh giá";
                if (listCT.Where(x => x.IdxetDuyetNavigation.IddeTai == listDetaiNghiemThu.ToList()[i].Id).Any())
                {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (!allDot.Any())
                return View();
            DotHienTai = await _serviceMoDot.GetEntity(x => x.Status == (int)MoDotStatus.Mo && x.Loai == (int)MoDotLoai.NghiemThuDeTai);
            if (DotHienTai == null)
                return View();
